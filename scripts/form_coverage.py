...
NOT_IDENTITY = re.compile(r"(reference|company|employer|supervisor|manager|contact|emergency|previous|prior)", re.I)
# Non-English labels for the four fields every form requires (mirrors content.js
# NAME_I18N_RE / EMAIL_I18N_RE / PHONE_I18N_RE).
NAME_I18N = re.compile(r"^\s*(名|姓|氏名|お名前|이름|성|vorname|nachname|familienname|voornaam|achternaam|naam|prénom|prenom|nom de famille|nom|nombre|apellidos?|nome|sobrenome|imię|nazwisko|förnamn|efternamn|fornavn|etternavn|etunimi|sukunimi)\s*$", re.I)
EMAIL_I18N = re.compile(r"^\s*(電子メール|メールアドレス|이메일|e-?mail(adres|adresse)?|correo( electrónico)?|courriel|endereço de e-?mail)\s*$", re.I)
PHONE_I18N = re.compile(r"^\s*(電話|電話番号|전화번호|telefon(nummer)?|telefoon(nummer)?|téléphone|telefone|teléfono|puhelin)\s*$", re.I)

RULES = [
    ("name",           lambda l: (bool(IDENTITY.search(l)) and not NOT_IDENTITY.search(l)) or bool(NAME_I18N.match(l))),
    ("email",          lambda l: bool(re.search(r"e-?\s?mail", l)) or bool(EMAIL_I18N.match(l))),
    ("phone",          lambda l: "phone" in l or bool(PHONE_I18N.match(l))),
    ("resume/file",    lambda l: bool(re.search(r"resume|cv\b|cover letter", l))),
    ("linkedin/url",   lambda l: bool(re.search(r"linkedin|portfolio|website|github|\burl\b", l))),
    ("address",        lambda l: bool(re.search(r"\b(street|address ?line|address ?1|mailing address|zip|postal)\b", l)) or l == "address"),
    ("city/state",     lambda l: bool(re.search(r"\bcity\b|\bstate\b|\bprovince\b|\bregion\b|location", l))),
    ("country/where",  lambda l: bool(re.search(r"(country|where (do|will) you (reside|live|work)|located in|intend to work)", l))),
    ("work auth",      lambda l: bool(re.search(r"(authoriz|eligible to work|legally.*work|right to work)", l))),
    ("sponsorship",    lambda l: bool(re.search(r"(sponsor|visa\b|h-?1b|immigration)", l))),
    ("salary",         lambda l: bool(re.search(r"(salary|compensation|pay range|wage|target|expected pay)", l))),
    ("how did you hear", lambda l: bool(re.search(r"how did you (hear|find|learn)|hear about (this|us|the)|where did you hear", l))),
    ("notice/start",   lambda l: bool(re.search(r"notice period|when (can|could) you start|available to start|start date", l))),
    ("english/lang",   lambda l: bool(re.search(r"(english|language).*(level|proficien|fluen)", l))),
    ("demographic",    lambda l: bool(re.search(r"(gender|sex\b|race|ethnic|hispanic|latino|veteran|disab|pronoun|self.?identif|national origin)", l))),
    ("consent/attest", lambda l: bool(re.search(r"(certif|attest|acknowledge|consent|i have read|i understand|\bterms\b|agree)", l))),
    ("years/experience", lambda l: bool(re.search(r"\byears?\b|experience|how many|how long", l))),
    ("former employee", lambda l: bool(re.search(r"(previously (worked|employed)|ever worked (at|for)|former (employee|employer)|currently employed by)", l))),
    ("age/18",         lambda l: bool(re.search(r"\b18\b|over 18|age\b", l))),
]
...
def classify(label: str, ftype: str) -> str:
    ...
    if AI_WORTHY.search(l) or ftype in ("textarea",) or len(l) > 20:
        return "→ AI"
