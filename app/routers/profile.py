...
def _lazy_tailor_for_job(user, job) -> None:
    ...
    try:
        ...
        # Paid = the full product (everything, incl. ATS tailoring). Both weekly and
        # monthly grant "pro"; get_tier() already maps any legacy premium/elite → pro.
        if get_tier(user.id, getattr(user, "email", None)) not in ("pro", "admin"):
            return
